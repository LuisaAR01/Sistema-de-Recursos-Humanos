...
def crear_usuario(username, password, rol, empleado_id=None):
    ...
    try:
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (username, password_hash, rol, empleado_id))
...
#Pa crear y obtener las evaluaciones
def crear_evaluacion(empleado_id, fecha, puntuacion, comentarios):
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        INSERT INTO evaluaciones (empleado_id, fecha, puntuacion, comentarios)
        VALUES (?, ?, ?, ?)
    """, (empleado_id, fecha, puntuacion, comentarios))
...
def obtener_evaluaciones():
    ...
    cursor.execute("""
        SELECT e.nombre, e.apellido, ev.fecha, ev.puntuacion, ev.comentarios
...
#Pa crear y obtener las capacitaciones
def crear_capacitacion(nombre, fecha, estado):
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        INSERT INTO capacitaciones (nombre, fecha, estado)
        VALUES (?, ?, ?)
    """, (nombre, fecha, estado))
...
def obtener_empleado_por_usuario(user_id):
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT empleado_id FROM usuarios WHERE id = ?
    """, (user_id,))
...
def obtener_evaluaciones_empleado(empleado_id):
    ...
    cursor.execute("""
        SELECT fecha, puntuacion, comentarios
        FROM evaluaciones
        WHERE empleado_id = ?
    """, (empleado_id,))
...
def obtener_nomina_empleado(empleado_id):
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT salario_base, afp, sfs, salario_neto, fecha
        FROM nomina
        WHERE empleado_id = ?
    """, (empleado_id,))
